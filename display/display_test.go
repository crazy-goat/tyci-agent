...
func TestTerminal_Summary_OutputsUsage(t *testing.T) {
	...
	if !strings.Contains(got, "[200]") {
		t.Errorf("expected cache read info, got %q", got)
	}
}
...
func TestTerminal_Summary_LongLine(t *testing.T) {
	...
	// For narrow terminals, aligner falls back to concatenation (no wrapping)
	// Should have at least 1 clearLine (from newBlock)
	if count := strings.Count(got, "\033[K"); count < 1 {
		t.Errorf("expected at least 1 clearLine sequence, got %d. Output: %q", count, got)
	}
}

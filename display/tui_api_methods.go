...
// ToolCallFailed reports whether the tool whose ToolCallEnd comes next failed.
// It is an optional sink capability so the public ToolCallEnd signature stays
// compatible with every display implementation. The dispatcher calls it next
// to ToolCallDuration, immediately before ToolCallEnd.
func (t *TUI) ToolCallFailed(failed bool) {
	t.pendingToolFailed = failed
}

func (t *TUI) ToolCallEnd(name, result string) {
	t.flushNow()
	d := t.pendingToolDuration
	failed := t.pendingToolFailed
	t.pendingToolDuration = 0
	t.pendingToolFailed = false
	t.post(tuiMsgBlock{
		kind:     "tool-end",
		content:  result,
		duration: d,

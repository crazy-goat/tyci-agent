...
// conversations (below) run on it, and the "wait" tool (tools/wait.go) can
// poll any job started here via SetJobWaiter's adapter in main(). It lives
// here, not in tools/tool.go, because tools deliberately does not import
// "jobs" (see tools.JobWaiter's doc comment) — main is the one layer allowed
// to depend on both.
